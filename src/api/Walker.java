...
public class Walker {
    ...
    private Pattern goPattern = Pattern.compile("(staircase)|(ladder)|((.*)?door)|(gate(.*)?)", Pattern.CASE_INSENSITIVE);
    /**
     ...
     */
    private boolean handleObstacle(Tile[] t) {
        ...
        for (GameObject go : ctx.objects.select(5).name(goPattern)) {                            //Changed to filter by name else all the filtering in the next part takes much too long to be considered effective..
            ...
            if (nextTile.floor() != p.tile().floor()) {
                if (go.type() == GameObject.Type.INTERACTIVE && go.actions().length>0) {
                    String actionString = (nextTile.floor() > p.tile().floor()) ? "Climb-up" : "Climb-down";
                    if (calcDist < distance && reachable(go)) {
                        for (String s : go.actions()) {
                            if (s != null && !s.equals("null") && s.contains(actionString)) {
                                obstacle = go;
        ...
        if (obstacle.inViewport()) {
            ...
            ctx.movement.stepWait(obstacle);                                                          //Can't see the obstacle, step towards it.
        }
        return false;
    }
    ...
    public boolean walkPath(Tile[] t) {
        ...
        if(nt.matrix(ctx).reachable()){
            ...
            return ctx.movement.stepWait(nt);
        }

        //System.out.println("NOT Randomized.");
        return ti.matrix(ctx).reachable() ? ctx.movement.stepWait(ti) : handleObstacle(t);
    }
